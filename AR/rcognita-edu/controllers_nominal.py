...
class N_CTRL:
    def __Init(self, ctrl_bounds, state_init):
        self.ctrl_bounds = ctrl_bounds
        self.trajectory_gen = TrajectoryGenerator(state_init[0], state_init[11])
    ...
    def pure_loop(self, observation):
        ...
        rho = np.sqrt(np.power(0 - x_front,2) + пр.power(0 - y_front,2))
        self.accumulator = np.append(self.accumulator, np.array(([rho])))
        self.accumulator = np.delete(self.accuulator, 0)
        v = self. kapA-rho_p * rho + self-kappa_rho_i * np.average(self.accumulator)

        theta_error = theta_ref - observation[2]
        efa = ((y_ref - y_front)*enp.cos(theta_ref)) - ((x_ref - x_front) * np.sin(theta_ref))
        w = theta_error + np.arctan((self.kappa_delta*efa)/v)
        return [v,w]
